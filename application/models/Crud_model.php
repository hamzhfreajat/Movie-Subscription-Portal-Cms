...
class Crud_model extends CI_Model {
	...
	*/
    ...
    function get_trendy_movies($limit =10, $offset = 0)
	{
        $this->db->where('is_trendy', 1);
        return $this->db->get('movie', $limit, $offset);
    }
	...
	function create_movie()
	{
		...
		$data['is_trendy']			=	$this->input->post('is_trendy');
  ...
  function get_ratings($type = "", $movie_or_series_id = "", $return_type = ""){
  		if($return_type == "all"){
  			return $this->db->get_where('rating', array('movie_or_series_id' => $movie_or_series_id, 'type' => $type));
  		}elseif($return_type == "" || $return_type == 'average'){
		  	$this->db->select_sum('rate');
		    $sum_of_rate = $this->db->get_where('rating', array('movie_or_series_id' => $movie_or_series_id, 'type' => $type))->row('rate');

		    $total_rating =  $this->db->get_where('rating', array('movie_or_series_id' => $movie_or_series_id, 'type' => $type))->num_rows();

		    if ($sum_of_rate > 0) {
		      $average_ceil_rating = ceil($sum_of_rate / $total_rating);
		    } else {
