...
class Browse extends CI_Controller {
	...
	function remove_rating($movie_or_series_id = ""){
		$this->db->where('movie_or_series_id', $movie_or_series_id);
		...
		redirect(site_url('index.php?browse/playmovie/'.$movie_or_series_id), 'refresh');
	}

	function user_rating($type = "", $movie_or_series_id = ""){
		if($movie_or_series_id > 0){
			$data['type'] = $type;
			$data['movie_or_series_id'] = $movie_or_series_id;
			$data['user_id'] = $this->session->userdata('user_id');
			$data['rate'] = htmlspecialchars($this->input->post('rate'));
			...
			$this->db->where('movie_or_series_id', $movie_or_series_id);
			$row = $this->db->get('rating');
			if($row->num_rows() > 0){
				$this->db->where('rating_id', $row->row('rating_id'));
