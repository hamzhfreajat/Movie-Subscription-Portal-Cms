...
<div class="container">
		...
		<div class="col-lg-12" style="margin: 0px 20px;">
			<form method="post" action="<?php echo base_url();?>index.php?home/signup">
                ...
                <div class="black_text">
                    <input type="text" name="name" style="padding: 10px; max-width: 400px; width: 100%; margin:10px 0px 5px; " autocomplete="off" required/>
                </div>
                <div style="margin:10px 0px 5px;">
                    <?php echo get_phrase('Email_Address');?>
                </div>
				<div class="black_text">
					<input type="email" name="email" style="padding: 10px; max-width: 400px; width: 100%;" autocomplete="on" required/>
				</div>
				<div style="margin:10px 0px 5px;">
					<?php echo get_phrase('Password');?>
				</div>
				<div class="black_text">
					<input id="password" type="password" name="password" style="padding: 10px; max-width: 400px; width: 100%;" required/>
				</div>
                <div style="margin:10px 0px 5px;">
                    تأكيد كلمة المرور
                </div>
                <div class="black_text">
                    <input id="confirm_password" type="password" name="password" style="padding: 10px; max-width: 400px; width: 100%;" required/>
                    <p class="password-error" style="color: red">كلمة المرور غير متطابقة</p>
                </div>
				<?php if(get_settings('recaptcha')): ?>
                ...
                <?php endif; ?>
				<button id="submit" type="submit"  class="btn btn-primary" style=" max-width: 150px; width: 100%; margin: 20px 0px;">
    ...
    <script>
        ...
        const passwodConfirm = document.getElementById("confirm_password");
        const error =   document.querySelector('.password-error');
        error.style.display = "none";
        passwodConfirm.addEventListener("keyup" , (e) => {
            clearTimeout(timer);
            timer = setTimeout(() => {
